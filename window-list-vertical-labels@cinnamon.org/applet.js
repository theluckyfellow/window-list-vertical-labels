    ...
    _getPreferredHeight(actor, forWidth, alloc) {
        ...
        if (this._applet.orientation == St.Side.TOP || this._applet.orientation == St.Side.BOTTOM ) {
            ...
            alloc.natural_size = this._applet._panelHeight;
             ...
             alloc.natural_size = naturalSize1 + this._label.get_preferred_height(forWidth)[1];
        }
    }
    ...
    _allocate(actor, box, flags) {
        ...
        if (this.labelVisible) {
            if (this._applet.orientation == St.Side.TOP || this._applet.orientation == St.Side.BOTTOM ) {
                ...
                yPadding = Math.floor(Math.max(0, allocHeight - naturalHeight));
                childBox.y1 = box.y1 + yPadding + 1 * global.ui_scale;
                childBox.y2 = childBox.y1 + Math.min(naturalHeight, allocHeight);
                if (direction === Clutter.TextDirection.LTR) {
                    // Reuse the values from the previous allocation
                    childBox.x1 = 2 * global.ui_scale;
                    childBox.x2 = box.x2;
                } else {
                    childBox.x2 = 2 * global.ui_scale;
                    childBox.x1 = box.x1;
                }
            }
...
class CinnamonWindowListApplet extends Applet.Applet {
    constructor(orientation, panel_height, instance_id) {
        ...
        this.settings = new Settings.AppletSettings(this, "window-list-vertical-labels@cinnamon.org", this.instance_id);
